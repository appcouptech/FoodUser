package food.user.demand.FCPojo.Fc_CouponObject;

public class CouponObject {


    private String d_images ;
    private String d_info ;
    private String id ;
    private String offer_code ;
    private String discount ;
    private String discount_type ;
    private String description ;
    private String enable_limit ;
    private String price_limit ;
    private String offer_upto ;
    private String use_count ;
    private String cards ;


    public String getD_images() {
        return d_images;
    }

    public void setD_images(String d_images) {
        this.d_images = d_images;
    }

    public String getD_info() {
        return d_info;
    }

    public void setD_info(String d_info) {
        this.d_info = d_info;
    }

    public String getOffer_code() {
        return offer_code;
    }

    public void setOffer_code(String offer_code) {
        this.offer_code = offer_code;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getDiscount() {
        return discount;
    }

    public void setDiscount(String discount) {
        this.discount = discount;
    }

    public String getDiscount_type() {
        return discount_type;
    }

    public void setDiscount_type(String discount_type) {
        this.discount_type = discount_type;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public String getEnable_limit() {
        return enable_limit;
    }

    public void setEnable_limit(String enable_limit) {
        this.enable_limit = enable_limit;
    }

    public String getPrice_limit() {
        return price_limit;
    }

    public void setPrice_limit(String price_limit) {
        this.price_limit = price_limit;
    }

    public String getOffer_upto() {
        return offer_upto;
    }

    public void setOffer_upto(String offer_upto) {
        this.offer_upto = offer_upto;
    }

    public String getUse_count() {
        return use_count;
    }

    public void setUse_count(String use_count) {
        this.use_count = use_count;
    }

    public String getCards() {
        return cards;
    }

    public void setCards(String cards) {
        this.cards = cards;
    }
}
